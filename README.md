...
* **Step 1: Intent Recognition & Initial Tool Selection**
    The system receives a request: *"Should we increase customer #12345’s credit limit to $10,000?"* The **Orchestrator Agent** identifies this requires internal data and external validation.

* **Step 2: Database Call (Internal Grounding)**
    The **Data Agent** executes a database call to retrieve the customer's payment history and current limit from an internal SQL database. 

* **Step 3: Agentic RAG (Policy Context)**
    The **Policy Agent** uses RAG to search the company’s "Credit Risk Manual." It doesn't just retrieve text; it evaluates if the retrieved policy specifically covers requests for $10,000+.
    * **Iteration:** If the retrieved text is vague, the agent autonomously modifies the search query to look for "high-value limit exceptions".
...
* **Step 5: The Iterative Control Loop (Condition Satisfaction)**
    ...
    * **Loop Trigger:** If the Credit Score API returns a "Thin File" (not enough data), the agent doesn't stop. It autonomously triggers a **Database Call** for the customer's linked savings account balance to see if that can substitute for a credit score.

* **Step 6: Structured Output Generation**
    Once all conditions are satisfied, the system uses **Pydantic** and a **JSON schema** to output the final decision in a machine-readable format for the banking core to execute.

### 2. Architectural Comparison

| Component | Role in the Use Case | Reliability Mechanism |
| :--- | :--- | :--- |
| **Agentic RAG** | Analyzes complex, shifting credit policy documents. | **Citation Enforcement:** Ensures the "Approval" is based on a specific manual section. |
| **Database Calls** | Fetches deterministic facts like account balance and tenure. | **JSON Schema:** Ensures the SQL result is correctly mapped to the agent's logic. |
| **API Calls** | Fetches live external risk data. | **Internal Retry Loops:** If the API times out, the agent retries or seeks alternative data. |
| **Autonomous Loop** | Decides if more data (like secondary accounts) is needed[cite: 30]. | **Temperature 0:** Ensures logical consistency rather than creative guessing. |

### 3. Why this is "Agentic"
Unlike a standard script that would simply crash if a credit score was missing, this **Agentic System** recognizes the "missing information" as a roadblock. It autonomously navigates back to the **Database** or **RAG context** to find a compliant workaround before generating the final structured output.
